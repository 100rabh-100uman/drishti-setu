...
try:
    ...
    from backend.utils.auth_utils import get_password_hash, verify_password, create_access_token, get_current_user, get_optional_current_user, ACCESS_TOKEN_EXPIRE_MINUTES
    from backend.utils.audit_logger import log_audit
except ImportError:
    from supabase_client import supabase
    from utils.auth_utils import get_password_hash, verify_password, create_access_token, get_current_user, get_optional_current_user, ACCESS_TOKEN_EXPIRE_MINUTES
...
@router.get("/get_users")
def get_users(department_id: Optional[int] = Query(None), current_user: Optional[str] = Depends(get_optional_current_user)):
    try:
        ...
        data = query.execute()
        if data.data:
            return {"users": data.data, "total": len(data.data)}
    except Exception:
        pass
    sample_users = [
        {"id": 1, "employee_id": "GP-ADM-001", "username": "Super Admin (DGP Office)", "department": "Gujarat Police Department", "department_name": "Gujarat Police Department", "role": "Admin", "status": "Active"},
        {"id": 2, "employee_id": "GP-TRA-042", "username": "Inspector V. Jadeja", "department": "Gujarat Traffic Branch", "department_name": "Gujarat Traffic Branch", "role": "Inspector", "status": "Active"},
        {"id": 3, "employee_id": "GP-INT-015", "username": "Sub-Inspector Priya Sharma", "department": "CID Crime Gujarat", "department_name": "CID Crime Gujarat", "role": "Analyst", "status": "Active"},
        {"id": 4, "employee_id": "GP-TEC-009", "username": "Field Tech Suresh Patel", "department": "Technical Operations Wing", "department_name": "Technical Operations Wing", "role": "Technician", "status": "Active"},
        {"id": 5, "employee_id": "GP-OPS-028", "username": "Officer Rajesh Vaghela", "department": "Command & Control Center", "department_name": "Command & Control Center", "role": "Operator", "status": "Active"}
    ]
