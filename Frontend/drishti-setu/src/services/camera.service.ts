...
class CameraService {
  ...
  private mapBackendCameraToModel(c: any): Camera {
    ...
    return {
      ...
      storage_type: c.storage_type || (c.camera_type === 'Analog' ? 'Local' : 'Cloud'),
      storage_days: Number(c.storage_days) || (c.camera_type === 'Analog' ? 30 : 60),
  ...
  async createCamera(formData: CameraFormData): Promise<CameraCreateResponse> {
    ...
    const payload: CameraCreatePayload = {
      ...
      storage_type: formData.storage_type || 'Cloud',
      storage_days: Number(formData.storage_days) || 30,
