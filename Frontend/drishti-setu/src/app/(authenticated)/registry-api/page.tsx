  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-700/60 pb-5">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-sky-400 tracking-wider uppercase mb-1">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-[#0d1527] border border-slate-800 rounded-xl p-4">
          <div className="text-xs font-semibold text-slate-400">REGISTERED UNITS</div>
        ...
        <div className="bg-[#0d1527] border border-emerald-500/30 rounded-xl p-4">
          ...
          <div className="text-2xl font-extrabold text-white mt-1 font-mono">{activeCount}</div>
        </div>
        <div className="bg-[#0d1527] border border-sky-500/30 rounded-xl p-4">
          <div className="text-xs font-semibold text-sky-400 flex items-center gap-1">
            <Video className="w-3.5 h-3.5" /> PTZ OPTICS
          </div>
          <div className="text-2xl font-extrabold text-white mt-1 font-mono">{types.PTZ || 2}</div>
        </div>
        <div className="bg-[#0d1527] border border-violet-500/30 rounded-xl p-4">
          ...
          <div className="text-2xl font-extrabold text-white mt-1 font-mono">{types.ANPR || 1}</div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 bg-[#0d1527] border border-slate-800 p-3.5 rounded-xl">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full bg-[#070d1d] border border-slate-800 rounded-lg pl-9 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-sky-500"
          />
        </div>
      </div>
                  ...
                  filteredCameras.map((camera, idx) => (
                    <tr key={camera.camera_id || idx} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <span className="px-2 py-0.5 text-[10px] font-mono rounded bg-slate-800 text-slate-200 border border-slate-700">
                          {camera.type || "IP PTZ"}
                        </span>
                      </td>
