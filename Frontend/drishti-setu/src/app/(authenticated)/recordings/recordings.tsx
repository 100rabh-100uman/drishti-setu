...
export default function RecordingsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#070d1d] text-slate-100 p-6 space-y-6">
      {/* ── Page Header ────────────────────────────────────────────── */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <div className="p-2.5 rounded-xl bg-gradient-to-br from-indigo-500/20 to-blue-600/20 border border-blue-500/30 text-blue-400">
              <Film className="w-6 h-6" />
            </div>
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
                CCTV Surveillance Archives & Playback
                <span className="text-xs font-mono font-medium px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/30">
                  Sentinel Engine
                </span>
              </h1>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <Link
            href="/camera-feed"
            className="flex items-center gap-2 px-3.5 py-2 rounded-lg text-xs font-medium bg-slate-800/80 hover:bg-slate-700 text-slate-200 border border-slate-700 transition"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-[#0b142c] border border-slate-800 rounded-xl p-4 shadow-sm relative overflow-hidden">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs text-emerald-400 font-medium">Standard</span>
          </div>
        ...
        <div className="bg-[#0b142c] border border-slate-800 rounded-xl p-4 shadow-sm relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400">Storage Utilized</span>
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-400">/ 5000 MB</span>
          </div>
        ...
        <div className="bg-[#0b142c] border border-slate-800 rounded-xl p-4 shadow-sm relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400">Playback Modes</span>
            <span className="p-1.5 rounded-lg bg-amber-500/10 text-amber-400 border border-amber-500/20">
              <Zap className="w-4 h-4" />
            </span>
          </div>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-white">1× / 16×</span>
            <span className="text-xs text-amber-400 font-medium">Decimated Fast View</span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1">
        ...
        <div className="bg-[#0b142c] border border-slate-800 rounded-xl p-4 shadow-sm relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400">Recorded Dates</span>
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-400 font-medium">Segments available</span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1">
      ...
      {activeRecording && (
        <div className="bg-[#0b142c] border border-blue-500/30 rounded-2xl overflow-hidden shadow-2xl">
          {/* Player Header */}
          <div className="bg-[#070d1d] px-5 py-3.5 border-b border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                  ...
                  <span className="text-slate-400 font-normal">| {activeRecording.camera_location}</span>
                </h3>
            ...
            <div className="flex items-center gap-3">
              {/* Speed Buttons */}
              <div className="flex items-center bg-[#050a16] p-1 rounded-xl border border-slate-800">
                <button
      ...
      <div className="bg-[#0b142c] border border-slate-800 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
        {/* Search Input */}
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-[#050a16] border border-slate-800 rounded-lg text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition"
          />
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="px-3 py-2 bg-[#050a16] border border-slate-800 rounded-lg text-xs text-slate-300 focus:outline-none focus:border-blue-500 transition cursor-pointer"
          >
      ...
      <div className="bg-[#0b142c] border border-slate-800 rounded-xl overflow-hidden shadow-sm">
        ...
        {loading ? (
          <div className="py-16 flex flex-col items-center justify-center text-slate-500 gap-3">
            ...
            <p className="text-xs">Loading CCTV storage index from backend...</p>
          </div>
        ) : filteredRecordings.length === 0 ? (
          <div className="py-16 flex flex-col items-center justify-center text-slate-500 gap-2">
            <AlertCircle className="w-8 h-8 text-slate-600" />
            <p className="text-xs font-medium text-slate-400">No recordings match your search filter</p>
            <button
              ...
              return (
                <div
                  ...
                  className={`p-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4 transition ${
                    isSelected ? "bg-blue-950/20 border-l-4 border-blue-500" : "hover:bg-slate-800/30"
                  }`}
                >
                  {/* Left: Camera & Segment Info */}
                  <div className="flex items-start gap-3.5">
                    <div className="p-2.5 rounded-xl bg-slate-800/80 border border-slate-700/60 text-slate-300 flex flex-col items-center justify-center min-w-[50px]">
                      <Video className="w-4 h-4 text-blue-400 mb-1" />
                      <span className="text-[10px] font-bold font-mono text-slate-400">{rec.camera_id}</span>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-cyan-950/30 border border-cyan-800/40 text-[11px] text-cyan-300 font-mono">
                      <Car className="w-3 h-3 text-cyan-400" />
                      <span>{rec.detections_summary.vehicles} Vehicles</span>
                    </div>
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-950/30 border border-emerald-800/40 text-[11px] text-emerald-300 font-mono">
                      <Users className="w-3 h-3 text-emerald-400" />
                      <span>{rec.detections_summary.persons} Persons</span>
                    </div>
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-purple-950/30 border border-purple-800/40 text-[11px] text-purple-300 font-mono">
