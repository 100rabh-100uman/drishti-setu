    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-[#0c162d] border border-cyan-200 dark:border-cyan-500/30 rounded-2xl p-4 shadow-xs">
          <div className="text-xs font-semibold text-cyan-700 dark:text-cyan-400">STATE CAMERA GRID</div>
          <div className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1 font-mono">
            {summary?.total_cameras ?? "500+"}
          </div>
          <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Total Monitored Nodes</div>
        </div>
        <div className="bg-white dark:bg-[#0c162d] border border-emerald-200 dark:border-emerald-500/30 rounded-2xl p-4 shadow-xs">
          ...
          <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Live Telemetry Rate</div>
        </div>
        <div className="bg-white dark:bg-[#0c162d] border border-amber-200 dark:border-amber-500/30 rounded-2xl p-4 shadow-xs">
          <div className="text-xs font-semibold text-amber-700 dark:text-amber-400">MAINTENANCE TICKETS</div>
          <div className="text-2xl font-extrabold text-amber-600 dark:text-amber-300 mt-1 font-mono">
            {summary?.active_maintenance_tickets ?? 0}
          </div>
          <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Pending Service Requests</div>
        </div>
        <div className="bg-white dark:bg-[#0c162d] border border-blue-200 dark:border-blue-500/30 rounded-2xl p-4 shadow-xs">
          ...
          <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Retention Compliant</div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 bg-white dark:bg-[#0c162d] border border-slate-200/80 dark:border-slate-800 p-3.5 rounded-2xl shadow-xs">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl pl-9 pr-3 py-1.5 text-xs text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500"
          />
        </div>
      </div>
      ...
      {loading ? (
        <div className="bg-white dark:bg-[#0c162d] border border-slate-200/80 dark:border-slate-800 rounded-2xl p-12 text-center text-slate-500 dark:text-slate-400 shadow-xs">
          <RefreshCw className="w-6 h-6 text-blue-600 dark:text-blue-400 animate-spin mx-auto mb-3" />
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                {filteredReports.length === 0 ? (
                  ...
                  filteredReports.map((report, idx) => (
                    <tr key={report.id || idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-slate-700 dark:text-slate-300">
                        <span className="px-2 py-0.5 text-[10px] rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                          {report.category}
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <button
                          ...
                          className="px-2.5 py-1 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-blue-600 dark:text-cyan-400 rounded text-xs font-semibold border border-slate-200 dark:border-slate-700 shadow-xs transition-colors inline-flex items-center gap-1.5"
                        >
