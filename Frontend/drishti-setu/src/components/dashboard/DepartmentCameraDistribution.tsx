  ...
  return (
    <div className="bg-white dark:bg-[#0c162d] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5 h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-[15px] font-bold text-slate-800 dark:text-white">Departments by Registered Cameras</h2>
        ...
        {data.map((dept, idx) => (
          <div key={idx} className="flex flex-col gap-1.5">
            <div className="flex justify-between items-end">
              <span className="text-xs font-semibold text-slate-700 dark:text-slate-300 truncate pr-4">{dept.name}</span>
              <span className="text-xs font-bold text-slate-900 dark:text-white">{dept.count.toLocaleString()}</span>
            </div>
