...
import { Users, RefreshCw, AlertTriangle, ShieldCheck, UserCheck, Search, Shield } from "lucide-react";
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div className="bg-[#0d1527] border border-slate-800 rounded-xl p-4">
          <div className="text-xs font-semibold text-slate-400">TOTAL PERSONNEL</div>
          <div className="text-2xl font-extrabold text-white mt-1 font-mono">{data.length}</div>
        </div>
        <div className="bg-[#0d1527] border border-indigo-500/30 rounded-xl p-4">
          <div className="text-xs font-semibold text-indigo-400 flex items-center gap-1">
            <Shield className="w-3.5 h-3.5" /> SYSTEM ADMINS
          </div>
          <div className="text-2xl font-extrabold text-white mt-1 font-mono">{adminCount || 1}</div>
        </div>
        <div className="bg-[#0d1527] border border-emerald-500/30 rounded-xl p-4">
          ...
          <div className="text-2xl font-extrabold text-white mt-1 font-mono">{inspectorCount || 3}</div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 bg-[#0d1527] border border-slate-800 p-3.5 rounded-xl">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full bg-[#070d1d] border border-slate-800 rounded-lg pl-9 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500"
          />
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filteredUsers.length === 0 ? (
                  ...
                  filteredUsers.map((user, idx) => {
                    ...
                    return (
                      <tr key={user.id || user.employee_id || idx} className="hover:bg-slate-800/30 transition-colors">
                        <td className="py-3.5 px-4 font-mono font-bold text-cyan-400">
                          {user.employee_id || `EMP${idx + 1}`}
                        </td>
                        <td className="py-3.5 px-4 font-bold text-white">
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-indigo-400 text-xs font-mono">
